...
const createMode = async (req, res, next) => {
	try {
		const { userId, platform, name, description, color, icon, energyLevel, focusRequired, category, blockedApps, blockedWebsites, cubeFace } = req.body;

		if (!userId || !platform || !name) {
			return res.status(400).json({ message: "UserId, platform, and name are required" });
		}
...
const updateMode = async (req, res, next) => {
	try {
		...
		const { name, description, color, icon, energyLevel, focusRequired, category, blockedApps, blockedWebsites, cubeFace } = req.body;
		...
		if (typeof focusRequired === "boolean") updateData.focusRequired = focusRequired;
...
const deleteMode = async (req, res, next) => {
	try {
		...
		const deletedMode = await modes.findByIdAndDelete(modeId);
		if (!deletedMode) {
			return res.status(404).json({ message: "Mode not found" });
		}
